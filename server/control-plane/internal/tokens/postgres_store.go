...
func (s *PostgresStore) Migrate(ctx context.Context) error {
	if err := s.applyMigrations(ctx, PostgresMigrations()); err != nil {
		return fmt.Errorf("apply token migrations: %w", err)
	}
	if err := s.verifySchema(ctx); err != nil {
		return fmt.Errorf("verify token schema: %w", err)
	}
	return nil
}
...
func (s *PostgresStore) verifySchema(ctx context.Context) error {
	...
	if count != requiredPostgresTokenColumns {
		return fmt.Errorf("api_tokens schema is missing required columns: found %d of %d", count, requiredPostgresTokenColumns)
	}
	return nil
}
