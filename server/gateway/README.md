...
Use `porthook doctor --control-plane http://localhost:8082 --admin-token-stdin` to check gateway and control-plane operations through the authenticated operator API. `porthook tunnels` requires `runtime_diagnostics`; `porthook history requests` requires `audit_history`.
