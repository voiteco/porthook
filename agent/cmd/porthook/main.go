...
const usageText = `usage: porthook login --server URL [--token TOKEN | --token-stdin]
       ...
       porthook admin tokens <create|list|revoke> [options]
       ...
       porthook tunnels <list|show> [options]
       porthook history <events|requests> [options]
       porthook export [options]
...
func runWithIO(args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) error {
	...
	case "tunnels":
		return runTunnelsCommand(args[1:], stdin, stdout, stderr)
	case "history":
		return runHistoryCommand(args[1:], stdin, stdout, stderr)
	case "export":
		return runOperationalExportCommand(args[1:], stdin, stdout, stderr)
	case "doctor":
		if wantsHelp(args[1:]) {
