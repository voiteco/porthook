...
The CLI also includes self-hosted control-plane token and reserved subdomain helpers:
...
| `PORTHOOK_CONTROL_PLANE_URL` | empty | Default control-plane API URL for `porthook tokens` and `porthook reserved` commands. |
| `PORTHOOK_CONTROL_ADMIN_TOKEN` | empty | Default admin token for `porthook tokens` and `porthook reserved` commands. Prefer `--admin-token-stdin` for shell history safety. |
